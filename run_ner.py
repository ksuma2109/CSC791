...
print("Training for NER")
data = pd.read_csv('train_ner.txt', sep=" ", header=None)
data[3], ner_mapping = pd.Series(data[3]).factorize()
ner_mapping = list(ner_mapping)
print(ner_mapping)
ner_cat = CategoricalDtype(categories=ner_mapping)
# print(ner_cat)
data = data[[0, 3]]
data.to_csv("train_ner.csv", index=False)
# data[2], chunk_mapping = pd.Series(data[2]).factorize()

data_test = pd.read_csv('test_ner.txt', sep=" ", header=None)
data_test = data_test.dropna()
# print(data_test.head(50))
data_test[3] = data_test[3].apply(lambda x: ner_mapping.index(x))
data_test = data_test[[0, 3]]
data.to_csv("test_ner.csv", index=False)
...
train_data, test_data, train_iter, test_iter, TEXT, LABEL = get_data('train_ner.csv','test_ner.csv' )
...
def train_model(model, train_iter, epoch):
	...
	for idx, batch in enumerate(train_iter):
		...
		word = batch.word[0]
		ner = batch.ner
		ner = torch.autograd.Variable(ner).long()
		optim.zero_grad()
		...
		# print(target.size())
		# print(target)
		loss = loss_fn(prediction, ner)
		num_corrects = (torch.max(prediction, 1)[1].view(ner.size()).data == ner.data).float().sum()
...
def eval_model(model, val_iter):
	...
	with torch.no_grad():
		for idx, batch in enumerate(val_iter):
			...
			prediction = torch.squeeze(prediction, dim=1)
			loss = loss_fn(prediction, ner)
			num_corrects = (torch.max(prediction, 1)[1].view(ner.size()).data == ner.data).sum()
			acc = 100.0 * num_corrects/len(batch)
			total_epoch_loss += loss.item()
			total_epoch_acc += acc.item()
...
for epoch in range(20):
    ...
    if epoch%1==0:
        ...
        torch.save(model, 'ner.pt')

print("Saving model for NER")
