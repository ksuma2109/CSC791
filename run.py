...
def train(config):
    ...
    for epoch in range(10000):
        for data in build_train_iterator():
            context_idxs = Variable(data['context_idxs'], volatile=True).to("cuda")
            ques_idxs = Variable(data['ques_idxs'], volatile=True).to("cuda")
            context_char_idxs = Variable(data['context_char_idxs'], volatile=True).to("cuda")
            ques_char_idxs = Variable(data['ques_char_idxs'], volatile=True).to("cuda")
            context_lens = Variable(data['context_lens'], volatile=True).to("cuda")
            y1 = Variable(data['y1'], volatile=True).to("cuda")
            y2 = Variable(data['y2'], volatile=True).to("cuda")
            q_type = Variable(data['q_type'], volatile=True).to("cuda")
            is_support = Variable(data['is_support'], volatile=True).to("cuda")
            start_mapping = Variable(data['start_mapping'], volatile=True).to("cuda")
            end_mapping = Variable(data['end_mapping'], volatile=True).to("cuda")
            all_mapping = Variable(data['all_mapping'], volatile=True).to("cuda")
