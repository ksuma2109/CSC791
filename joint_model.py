...
class JointModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.pos = EncoderRNN(self.word_dim, config.hidden, 1, True, True, 1-config.keep_prob, False);
        self.linear_pos =  nn.Sequential(
                nn.Linear(config.hidden*2, config.hidden),
                nn.ReLU()
            )

        self.ner = EncoderRNN(config.hidden, config.hidden, 1, True, True, 1-config.keep_prob, False);
        self.linear_ner =  nn.Sequential(
                nn.Linear(config.hidden*2, config.hidden),
                nn.ReLU()
            )

        self.dp = EncoderRNN(config.hidden, config.hidden, 1, True, True, 1-config.keep_prob, False);
        self.linear_dp =  nn.Sequential(
                nn.Linear(config.hidden*2, config.hidden),
                nn.ReLU()
            )

        self.se = EncoderRNN(config.hidden, config.hidden, 1, True, True, 1-config.keep_prob, False);
        self.linear_se =  nn.Sequential(
                nn.Linear(config.hidden*2, config.char_hidden+self.word_dim),
                nn.ReLU()
            )

        self.rnn = EncoderRNN(2*(config.char_hidden+self.word_dim), config.hidden, 1, True, True, 1-config.keep_prob, False)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        context_pos = self.pos(context_word, context_lens)
        context_pos = self.linear_pos(context_pos)

        context_ner = self.ner(context_pos, context_lens)
        context_ner = self.linear_ner(context_ner)

        context_dp = self.dp(context_ner, context_lens)
        context_dp = self.linear_dp(context_dp)

        context_se = self.se(context_dp, context_lens)
        context_se = self.linear_se(context_se)

        question_pos = self.pos(ques_word)
        question_pos = self.linear_pos(question_pos)

        question_ner = self.ner(question_pos)
        question_ner = self.linear_ner(question_ner)

        question_dp = self.dp(question_ner)
        question_dp = self.linear_dp(question_dp)

        question_se = self.se(question_dp)
        question_se = self.linear_se(question_se)

        context_input = torch.cat([context_output, context_se], dim=2)
        ques_input = torch.cat([ques_output, question_se], dim=2)

        # print(context_input.shape)
        # print(ques_input.shape)
        # print(context_lens.shape)
        context_output = self.rnn(context_input, context_lens)
        ques_output = self.rnn(ques_input)
